...
int main(int argc, char **argv)
{
  ...
  checkForCgError("could not get eyePositionW parameter");

  myCgVertexParam_modelToWorld =
	  cgGetNamedParameter(myCgVertexProgram, "modelToWorld");
  checkForCgError("could not get modelToWorld parameter");

  myCgVertexParam_lightColor =
	  cgGetNamedParameter(myCgVertexProgram, "lightColor");
  checkForCgError("could not get lightColor parameter");

  myCgVertexParam_Kd =
	  cgGetNamedParameter(myCgVertexProgram, "Kd");
  checkForCgError("could not get Kd parameter");

  myCgVertexParam_Ks =
	  cgGetNamedParameter(myCgVertexProgram, "Ks");
  checkForCgError("could not get Ks parameter");

  myCgVertexParam_shininess =
	  cgGetNamedParameter(myCgVertexProgram, "shininess");
  checkForCgError("could not get shininess parameter");
  ...
  checkForCgError("getting reflectivity parameter");
...
static void display(void)
{
  ...
  cgSetParameter3f(myCgVertexParam_lightPosition, 0, 5, 10);
  ...
  checkForCgError("setting eye positionW");

  cgGLSetStateMatrixParameter(myCgVertexParam_modelToWorld,
	  CG_GL_MODELVIEW_PROJECTION_MATRIX,
	  CG_GL_MATRIX_IDENTITY);
  checkForCgError("setting modelToWorld matrix");

  cgSetParameter3f(myCgVertexParam_lightColor, 1, 1, 1);
  checkForCgError("setting light color");

  cgSetParameter3f(myCgVertexParam_Kd, 0.8, 0.5, 0.3);
  checkForCgError("setting Kd");

  cgSetParameter3f(myCgVertexParam_Ks, 0.2, 0.2, 0.2);
  checkForCgError("setting Ks");

  cgSetParameter3f(myCgVertexParam_shininess, 0.5, 0.5, 0.5);
  checkForCgError("setting shininess");
  ...
  cgSetParameter1f(myCgFragmentParam_reflectivity, 1);
  checkForCgError("setting reflectivity");
